...
<div class="flex min-h-screen flex-col">
    <header class="sticky top-0 z-40 border-b border-base-300/80 bg-base-100/85 shadow-sm backdrop-blur-lg">
        <div class="grid h-16 grid-cols-[auto_1fr_auto] items-center gap-4 px-4 sm:px-6 lg:px-10">
            <a href="{{ route('teacher.dashboard') }}" class="flex shrink-0 items-center gap-2.5">
                <span
                    class="flex size-9 items-center justify-center rounded-lg bg-primary text-base font-bold text-white">K</span>
                    ...
                    @else
                        ...
                        <div class="relative" x-data="{ open: false }" @click.outside="open = false"
                            ...
                            <button @click="open = !open"
                                ...
                                <x-icon name="heroicon-o-{{ $group['icon'] }}" class="size-4 opacity-70"/>
                                {{ $group['label'] }}
                                <x-icon name="heroicon-o-chevron-down" class="size-3.5 opacity-60 transition-transform"
                                        x-bind:class="open ? 'rotate-180' : ''"/>
                            </button>

                            <div x-cloak x-show="open" x-transition.opacity.duration.150ms
                                 class="absolute left-0 top-full mt-1.5 w-48 rounded-xl border border-base-200 bg-base-100 p-1.5 shadow-lg">
                ...
                <div class="relative" x-data="{ open: false }" @click.outside="open = false"
                    ...
                    >
                        @if($user->avatar_url)
                            <img src="{{ $user->avatar_url }}" alt="{{ $user->full_name }}"
                                 class="size-8 rounded-full object-cover ring-2 ring-primary/20"/>
                        @else
                            <span
                                class="flex size-8 items-center justify-center rounded-full bg-primary text-xs font-bold text-white">{{ $initials }}</span>
                        @endif
                        <span
                            class="hidden text-sm font-medium text-base-content/80 sm:block">{{ $user->full_name }}</span>
                        <x-icon name="heroicon-o-chevron-down"
                                class="size-4 text-base-content/40 transition-transform duration-200"
                                x-bind:class="open ? 'rotate-180' : ''"/>
                    </button>
                    ...
                    >
                        ...
                        <div class="flex items-center gap-3 border-b border-base-200 bg-base-200/40 px-4 py-3.5">
                            @if($user->avatar_url)
                                <img src="{{ $user->avatar_url }}" alt="{{ $user->full_name }}"
                                     class="size-10 shrink-0 rounded-full object-cover ring-2 ring-primary/20"/>
                            @else
                                <span
                                    class="flex size-10 shrink-0 items-center justify-center rounded-full bg-primary text-sm font-bold text-white">{{ $initials }}</span>
                            @endif
                            <div class="min-w-0">
                        ...
                        <div class="border-t border-base-200 p-1.5">
                            <a href="{{ route('teacher.dashboard') }}"
                               class="flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm font-medium text-base-content/75 transition hover:bg-primary/10 hover:text-primary">
                        ...
                        <div class="border-t border-base-200 p-1.5">
                            <form method="POST" action="{{ route('auth.logout') }}">
                                @csrf
                                <button type="submit"
                                        class="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-sm font-medium text-error transition hover:bg-error/10">
    ...
    {{-- İçerik --}}
    <main
        class="flex-1" @style(['width: 100%' => config('theme.full_bleed', true), 'max-width: 72rem; margin-inline: auto; width: 100%' => ! config('theme.full_bleed', true)])>
        <div @style([
