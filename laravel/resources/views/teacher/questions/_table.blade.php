...
<nav class="mb-4 flex flex-wrap items-center gap-2 px-1 py-2 text-sm">
    <a href="{{ route('teacher.questions.index') }}" class="inline-flex items-center gap-1.5 rounded-md px-2.5 py-1.5 font-medium text-primary transition-colors hover:bg-primary/10">
        <x-icon name="heroicon-o-home" class="size-4" />
        Kök
    </a>
    @foreach ($breadcrumbs as $crumb)
        <x-icon name="heroicon-s-chevron-right" class="size-4 text-base-content/30" />
        <a href="{{ route('teacher.questions.index', ['folder_id' => $crumb['id']]) }}" class="rounded-md px-2.5 py-1.5 font-medium text-base-content/70 transition-colors hover:bg-base-200 hover:text-base-content">
            {{ $crumb['name'] }}
        </a>
    @endforeach
</nav>
