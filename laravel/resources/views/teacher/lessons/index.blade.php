...
>
    ...
    <x-teacher.card :padding="false">
        ...
        <nav class="mb-4 flex flex-wrap items-center gap-2 border-b border-base-300 px-5 py-3.5 text-sm">
            <a href="{{ route('teacher.lessons.index') }}" class="inline-flex items-center gap-1.5 rounded-md px-2.5 py-1.5 font-medium text-primary transition-colors hover:bg-primary/10">
            ...
            @foreach ($folders['breadcrumbs'] as $crumb)
                ...
                <a href="{{ route('teacher.lessons.index', ['folder_id' => $crumb['id']]) }}" class="rounded-md px-2.5 py-1.5 font-medium text-base-content/70 transition-colors hover:bg-base-200 hover:text-base-content">
                    {{ $crumb['name'] }}
                </a>
            @endforeach
        </nav>
            ...
            <x-teacher.table :headers="['Ad', 'Tip / Say', 'Video', 'Yayım', 'Sıra', 'Yaradılıb']">
                ...
                @foreach ($lessons as $lesson)
                    <tr
                        ...
                        data-open-url="{{ route('teacher.lessons.show', $lesson['content_id']) }}"
                        data-edit-url="{{ route('teacher.lessons.edit', $lesson['content_id']) }}"
                        @contextmenu.prevent="openRowContextMenu($event, 'lesson', $el)"
                    >
